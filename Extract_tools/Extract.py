...
for i in range(1, 51) :
    ...
    #f = open('Result_4/ThrMuMMEHS_min29_'+str(i)+'.txt') 원본
    f = open('Result_7/Thrmummevmp_'+str(i)+'.txt')
    b = f.readlines()
...
print('평균 Energy Consumption', round (sum(Energy) / len(Energy),5))
print('-----------')
print('Best/Worst active 시간', max(Active_time), min(Active_time), max(Active_time)-min(Active_time))
print('Best/Worst 최대 호스트', max(MaxHost), min(MaxHost), max(MaxHost)-min(MaxHost))
print('Best/Worst 마이그레이션', max(Migration), min(Migration), max(Migration)-min(Migration))
print('Best/Worst 호스트재시작', max(Restart), min(Restart), max(Restart)-min(Restart))
...
print('Best/Worst Energy', max(Energy), min(Energy), max(Energy)-min(Energy))